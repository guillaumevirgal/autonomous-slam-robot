...
#define TEST_DUTY           0.01f    // small, safe fixed duty  
...
static void control_task(void *arg)
{
    ...
    while (1) {
        ...
        float duty_a;
        if (phase == 0 && duty_a <= 0.24) {
            duty_a = duty_a + step * TEST_DUTY;      // positive: note wheel direction + measured sign
        } else if (phase == 2) {
            duty_a = -duty_a - step * TEST_DUTY;     // negative
            step = step + 1;
        } else {
...
void app_main(void)
{
    ESP_LOGI(TAG, "Deadband motor check");
    ESP_LOGI(TAG, "duty cycles +0.01 / coast / -0.01 / coast, 3s each");
